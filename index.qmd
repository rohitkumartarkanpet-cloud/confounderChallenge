...
Most people would feel safer picking Doc Dreamy. But looks can hide what is really going on in the data. Here we walk through real-looking data for these two doctors and see how a hidden factor can flip the story.
...
- **Observational data**: patients picked their surgeon in a realistic way (based on looks, waiting time, and personal preference).
...
for size, alpha in [(90, 0.9), (150, 0.15)]:
    ax.scatter(
        ...
        color=dreamy_color,
        alpha=alpha,
        label="Doc Dreamy" if size == 90 else None,
        zorder=3,
    )
    ax.scatter(
        ...
        color=duck_color,
        alpha=alpha,
        label="Doc Duck" if size == 90 else None,
        zorder=3,
    )
...
ax.set_xlabel("Patient number", color="white", fontsize=12)
ax.set_ylabel("Post-surgical symptom score (lower is better)", color="white", fontsize=12)
ax.set_title("Post-surgical symptom scores – observational data", color="white", fontsize=14, pad=12)
...
x_center_r = n_patients_rnd * 0.75
ax.text(
    x_center_r,
    mean_dreamy_r,
...
ax.set_xlabel("Patient number", color="white", fontsize=12)
ax.set_ylabel("Post-surgical symptom score (lower is better)", color="white", fontsize=12)
ax.set_title("Post-surgical symptom scores – randomized assignment", color="white", fontsize=14, pad=12)
...
ax.set_xlabel("Initial patient severity", color="white", fontsize=12)
ax.set_ylabel("Post-surgical symptom score (lower is better)", color="white", fontsize=12)
ax.set_title("Post-surgical symptom scores by initial severity – observational data", color="white", fontsize=14, pad=12)
...
Now the comparison is made for patients who started with similar severity. This reduces the bias from the common cause.
...
The three figures and the summary tables together tell the story:
